...
- The prune deletes from FastMCP's private `_tool_manager._tools` dict, so `memora` pins `mcp>=1.0.0,<1.28` and runs a startup **attestation** through the actual `tools/list` + `call_tool` path that refuses to start if the installed SDK routes listing/dispatch elsewhere (private-implementation drift). Bumping the upper bound requires re-running `tests/test_tool_profile.py`.
