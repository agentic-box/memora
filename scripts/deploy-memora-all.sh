#!/usr/bin/env bash
# Full deploy of the live memora-all container (on DEPLOY_HOST) to v0.5.5: fetch +
...
#  1. git fetch + checkout the v0.5.5 tag in the deploy-host checkout, docker build.
...
#  5. Wait for GET /health, check it reports version 0.5.5 (proves the new
