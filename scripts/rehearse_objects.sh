...
track() { echo "$1 $2" >> "$OBJECTS"; }   # track KIND ID  (container|volume|anon|image)
...
remove_one() {  # remove_one KIND ID -- only when it is still this run's; by ID
  ...
  case "$kind" in
    ...
    image)
      "$RT" image exists "$id" 2>/dev/null || return 0
      [ "$(label_of image "$id")" = "$RUN_ID" ] || { echo "keep image $id: not labelled $RUN_ID"; return 0; }
      # untag each tag (never -f: a force would also remove containers using it)
      for t in $("$RT" image inspect "$id" --format '{{range .RepoTags}}{{.}} {{end}}'); do
        "$RT" rmi "$t" >/dev/null 2>&1 || { echo "keep image $id: $t still in use"; return 0; }
      done
      "$RT" image exists "$id" 2>/dev/null && { "$RT" rmi "$id" >/dev/null 2>&1 || true; }
      echo "removed image $id" ;;
  esac
}
...
teardown() {  # teardown [LIST] -- containers first, then volumes, anonymous volumes, images
  ...
  [ -s "$list" ] || return 0
  for kind in container volume anon image; do
