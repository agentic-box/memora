 ...
 * Shipped module (memora/graph/_graph_limit.mjs). public/_graph_limit.mjs
 * is a symlink. Tests MUST import this file (no mirrors). Mirrors the
 * /api/graph ?limit= contract (default cap 2000, hard max 5000).
 */
...
export function formatTruncationBanner({ truncated, shown, total }) {
  ...
  const shownOk = Number.isFinite(n) && n >= 0;
  const m = Number(total);
  const totalOk = Number.isFinite(m) && m >= 0;
  if (shownOk && totalOk) {
    return { visible: true, text: `showing ${n} of ${m} nodes` };
  }
  if (shownOk) {
    return { visible: true, text: `showing ${n} nodes; more available` };
  }
  return { visible: true, text: "more nodes available" };
}
