...
memora-all's graph API returns exactly what the Pages viewer returns.

- `/api/graph` is now built by a port of the Pages code (`memora/graph/payload.py`): document fragments with `?docs=1`, typed and scored edges, supersession lineage and its flags, the same labels, colours, sizes and clusters. Both viewers now show the same graph for the same store.
- The default edge threshold is 0.40, as on Pages (`?min_score=` still overrides it).
- One deliberate difference: rows of an unfinished import are not shown.

## 0.5.3

Correct graph clusters, and nightly compares of the local primaries.

- Graph clusters are computed as the Pages viewer computes them (over the stored cross-references), instead of over every pair of memories. Clusters no longer collapse into a few giant groups, and the graph builds in well under a second instead of up to minutes.
- `scripts/nightly_compare.sh` runs from cron on the deploy host: a nightly compare of every live local primary against D1, a weekly barrier compare on Sunday, one store at a time under a lock, with a log line per store and a non-zero exit on any diff, halt or refusal.

## 0.5.2

- `reconcile --accept` can accept an open D1 write intent again: the evidence digest now covers what was read, not when it was read, and the accept re-reads D1 itself, refusing when D1 changed or could not be read. Digests shown by earlier versions no longer match; show the intents again.

## 0.5.1

Graph viewer served by memora-all, and embedding fixes.

- memora-all serves the graph UI (and the force-graph 2D/3D view) for every configured store, selected with `?db=`, behind its own graph token and a login form; it is published only on a private address. Edits go through the normal store path, so a local primary's edits replicate; a frozen store is read-only.
- A search never rebuilds or writes embeddings any more. The embedding fingerprint no longer includes the embedding server's host, so moving that server no longer triggers full rebuilds. A real model mismatch refuses semantic search and shows `embeddings.repair_needed` in `/health/db`; the repair is the explicit `memory_rebuild_embeddings`.
- Stores written before this release, with no recorded model, are searched when their vectors fit the current model; `memory_verify_integrity(record_model=true)` records it.
- Deploy preflights: `python -m memora.embedding_preflight` refuses a deploy under which any store would refuse semantic search, and the import check reads a live local primary read-only.
...
- **D1 is written only by memora:** the Pages viewer is read-only (edits answer 405), retired scripts that wrote D1 are inert, and `scripts/d1_write_guard.py` blocks new D1 writers in CI and before every Pages deploy.
- Client configuration tools: `scripts/audit_configs.py` finds configurations that reach D1 directly, and `scripts/repoint_mcp_config.py` repoints MCP clients to memora-all.
- `scripts/apply_backfill_47.py` applies an approved issue #47 backfill preview, row by row, with verification.
