...
- New `scripts/rehearse_deploy.sh` and `docs/deploy-rehearsal.md` run the real `deploy-memora-all.sh` against rootless podman on a rehearsal host, with local stores only. It covers: the production-shaped old container (anonymous `/data`, WAL sidecars, an intent journal); the migration; the new container (960m, tokens); a no-op second deploy; rollback and recopy; freeze/thaw through the live admin routes. 49 checks, all passing on server2.
- `deploy-memora-all.sh` refuses any non-production value unless `DEPLOY_REHEARSAL=1` is set. With the sentinel, the whole target must be rehearsal-scoped: localhost only, `-rh` names, not port 8920, config and env under `DEPLOY_REHEARSAL_ROOT`. Every run prints its effective target first. The rehearsal labels everything it creates with a per-run id (the deploy's objects via `DEPLOY_LABELS`, rehearsal-only). Its cleanup removes a container or volume only when the label is a run id from its own ledger and the name fits. Anonymous volumes must also have been created inside that run's window. `scripts/rehearse_cleanup_selftest.sh` checks every rule on the real runtime.
