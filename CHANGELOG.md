...
- Admin-token files (`memora-instance.sh` `<instance>.admin-token`, `deploy-memora-all.sh` `all.admin-token`): an existing file must be a regular file, not a symlink, owned by the user and mode 0600; otherwise the launcher refuses before touching the container and says to remove it and re-mint. It is never chmod-ed into shape. The server reads the token from the environment only.
