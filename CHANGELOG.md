...
- Production finding (deploy-host v0.5.0). After the M1 moved hosts (same Ollama bge-m3), every store's stored fingerprint mismatched, because it carried the embedding endpoint's host (`backend|model|host|repr`).
  - The first search on the seeded local `gamma` rebuilt all 238 embeddings.
  - A search on the D1 store `alpha` rebuilt all 615 on D1.
  - The D1 store `beta` re-tried a rebuild on every search behind a leftover rebuild lease, logging "Embedding model changed" each time.
...
- `scripts/deploy-memora-all.sh` publishes the graph only on deploy-host's
  Tailscale address (100.64.0.10:8766 → 8765; `DEPLOY_GRAPH_BIND` /
...
absent, memora-all runs as before. The first store is `gamma`. The entries below,
...
- **Replicator timing** (leader 7762), from the environment:
  ...
  all three from `instances/all.env`, validated first. The `gamma` pilot runs
...
### Local-primary X3: a maintenance venue on deploy-host, and the primary lock as a barrier
...
- Round 5 (review 7845): a lock file is never a symlink. The service lock and every store's primary lock are opened with `O_NOFOLLOW`, must be a regular file on their directory's device, and are checked with `lstat` at every boundary; only the directory is resolved.
...
- New `scripts/rehearse_deploy.sh` and `docs/deploy-rehearsal.md` run the real `deploy-memora-all.sh` against rootless podman on a rehearsal host, with local stores only. It covers: the production-shaped old container (anonymous `/data`, WAL sidecars, an intent journal); the migration; the new container (960m, tokens); a no-op second deploy; rollback and recopy; freeze/thaw through the live admin routes. 47 checks, all passing on build-host.
- `deploy-memora-all.sh` refuses any non-production value unless `DEPLOY_REHEARSAL=1` is set. With the sentinel, the whole target must be rehearsal-scoped: localhost only, `-rh` names, not port 8920, config and env under `DEPLOY_REHEARSAL_ROOT`. Every run prints its effective target first. Every rehearsal object has a per-run name and label and an ID captured from a successful create (`scripts/rehearse_objects.sh`). The deploy's objects are labelled via `DEPLOY_LABELS`, rehearsal-only. Each run tears down only its own recorded objects, by ID after re-checking the label, and never by name. Images are never removed by ID: only the tags the run recorded are untagged, each while it still names the run's labelled image. `scripts/rehearse_cleanup_selftest.sh` and `scripts/rehearse_survival_check.sh` check this on the real runtime.
