...
def test_the_defaults_are_the_production_deploy(deploy):
    """Unparameterised: ssh to deploy-host, docker, the v0.5.4 checkout, memora-all
    ...
    assert ("deploy target: host=deploy-host runtime=docker container=memora-all volume=memora-all-data "
            "image=memora:latest port=8920 graph=100.64.0.10:8766 tag=v0.5.4 secrets=~/.config/memora-lp") in proc.stdout
    assert any(t.startswith("git ") and "checkout v0.5.4" in t for t in tools)
    run = _new_container_run(calls)
...
class TestRemoteArguments:
    ...
    def test_the_remote_command_is_one_word_carrying_every_parameter(self, deploy, tmp_path):
        ...
        assert params[0] == "v0.5.4" and params[3] == "docker" and params[4] == "memora-all"
